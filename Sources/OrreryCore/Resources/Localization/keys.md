...
| `account.loginReadyHintGemini` | Hint printed by the shell function just before `command gemini` is launched for account-add. Unlike the claude one, the user really does have to quit: gemini has no `auth_success` hook, so `_account-add-finalize` only runs once the process exits. Deliberately names no slash command — gemini's quit binding is not something orrery should assert. |
