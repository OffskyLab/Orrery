...
public enum L10n {
    ...
    public enum Create {
        public static var abstract: String {
            isChinese ? "建立新的 orrery 環境" : "Create a new orrery environment"
        }
        ...
        public static func alreadyExists(_ name: String) -> String {
            isChinese
                ? "環境 '\(name)' 已存在。請使用其他名稱或先執行 'orrery delete \(name)'。"
                : "Environment '\(name)' already exists. Use a different name or 'orrery delete \(name)' first."
        }
        ...
        public static func firstEnvCreated(_ name: String) -> String {
            isChinese
                ? "\n已建立第一個環境 — 自動啟用 '\(name)'。\n執行 'orrery use \(name)' 以套用到此 shell。"
                : "\nFirst environment created — activating '\(name)' automatically.\nRun 'orrery use \(name)' to apply it to this shell."
        }
        ...
        public static var noToolSelected: String {
            isChinese
                ? "未選擇任何工具 — 可用 'orrery tools add <env>' 之後再加入。"
                : "No tool selected — use 'orrery tools add <env>' to add one later."
        }
    ...
    public enum Info {
        public static var abstract: String {
            isChinese ? "顯示 orrery 環境的詳細資訊" : "Show details of an orrery environment"
        }
        public static var nameHelp: String {
        ...
        public static var noActive: String {
            isChinese
                ? "沒有啟用的環境。請指定名稱或先執行 'orrery use <name>'。"
                : "No active environment. Specify a name or run 'orrery use <name>' first."
        }
        ...
        public static var defaultInfo: String {
            isChinese
                ? "名稱：       origin\n描述：       原始系統環境。切換到 'origin' 等同於離開 Orrery — 工具使用系統全域設定，不套用任何隔離或環境變數。"
                : "Name:        origin\nDescription: Your system origin environment. Switching to 'origin' exits Orrery — tools fall back to system-wide config, with no isolation or per-environment variables applied."
        }
    }
    ...
    public enum List {
        public static var abstract: String {
            isChinese ? "列出所有 orrery 環境" : "List all orrery environments"
        }
        public static var empty: String {
            isChinese
                ? "找不到任何環境。使用以下指令建立：orrery create <name>"
                : "No environments found. Create one with: orrery create <name>"
        }
    ...
    public enum Rename {
        public static var abstract: String {
            isChinese ? "重新命名 orrery 環境" : "Rename an orrery environment"
        }
        public static var nameHelp: String {
    ...
    public enum EnvVar {
        ...
        public static var envHelp: String {
            isChinese
                ? "環境名稱（預設為 ORRERY_ACTIVE_ENV）"
                : "Environment name (defaults to ORRERY_ACTIVE_ENV)"
        }
        public static var noActive: String {
            isChinese
                ? "沒有啟用的環境。請先執行 'orrery use <name>'，或使用 -e <name>。"
                : "No active environment. Run 'orrery use <name>' first, or use -e <name>."
        }
    ...
    public enum Tools {
        ...
        public static var defaultNotSupported: String {
            isChinese
                ? "'origin' 是原始系統環境，不支援工具設定。請先執行 'orrery create <name>' 建立環境。"
                : "'origin' is the system origin environment and does not support tool configuration. Run 'orrery create <name>' to create an environment."
        }
    }
    ...
    public enum Use {
        ...
        public static var needsShellIntegration: String {
            isChinese
                ? "error: 'orrery use' 需要 shell 整合。\n請執行 'orrery setup' 安裝後，重新啟動終端機。\n"
                : "error: 'orrery use' requires shell integration.\nRun 'orrery setup' to install it, then restart your terminal.\n"
        }
        public static var switchedToOrigin: String {
            isChinese
                ? "已回到原始環境，如需回到 Orrery 管理環境，請再次使用 orrery use <name> 即可。"
                : "Returned to the origin environment. To re-enter an Orrery environment, run: orrery use <name>"
        }
    }
    ...
    public enum Which {
        ...
        public static var noActive: String {
            isChinese
                ? "沒有啟用的環境。請先執行 'orrery use <name>'。"
                : "No active environment. Run 'orrery use <name>' first."
        }
    }
    ...
    public enum Setup {
        public static var abstract: String {
            isChinese
                ? "安裝 orrery shell 整合（使用：eval \"$(orrery setup)\"）"
                : "Install orrery shell integration (use: eval \"$(orrery setup)\")"
        }
        ...
        public static func alreadyPresent(_ path: String) -> String {
            isChinese
                ? "orrery: shell 整合已存在於 \(path)\n"
                : "orrery: shell integration already present in \(path)\n"
        }
        public static func addedTo(_ path: String) -> String {
            isChinese
                ? "orrery: 已加入到 \(path)\n"
                : "orrery: added to \(path)\n"
        }
        public static func failedToWrite(_ path: String, _ error: String) -> String {
            isChinese
                ? "orrery: 寫入 \(path) 失敗：\(error)\n"
                : "orrery: failed to write \(path): \(error)\n"
        }
        ...
        public static func migratedRc(_ path: String) -> String {
            isChinese
                ? "orrery: 已將 \(path) 中的 eval 改為 source\n"
                : "orrery: migrated \(path) from eval to source\n"
        }
    }
    ...
    public enum Update {
        public static var abstract: String {
            isChinese ? "將 orrery 更新至最新版本" : "Update orrery to the latest version"
        }
        public static var upgrading: String {
            isChinese ? "正在更新 orrery…" : "Upgrading orrery…"
        }
        public static var unsupportedPlatform: String {
            isChinese
                ? "不支援的平台。請手動更新：https://github.com/OffskyLab/Orrery/releases"
                : "Unsupported platform. Update manually: https://github.com/OffskyLab/Orrery/releases"
        }
        public static func notice(current: String, latest: String) -> String {
            isChinese
                ? "orrery: 有新版本可用 \(current) → v\(latest)  執行: orrery update"
                : "orrery: update available \(current) → v\(latest)  run: orrery update"
        }
    }
    ...
    public enum Init {
        public static var abstract: String {
            isChinese
                ? "輸出 shell 整合腳本（加入 shell rc：eval \"$(orrery init)\"）"
                : "Output shell integration script (add to shell rc: eval \"$(orrery init)\")"
        }
    }
    ...
    public enum Resume {
        ...
        public static var noIndex: String {
            isChinese
                ? "請指定 session index。用 orrery sessions 查看列表。"
                : "Please specify a session index. Run orrery sessions to see the list."
        }
    ...
    public enum Run {
        ...
        public static var noCommand: String {
            isChinese
                ? "請指定要執行的指令。例如：orrery run -e work claude --resume <id>"
                : "No command specified. Example: orrery run -e work claude --resume <id>"
        }
    }
    ...
    public enum Memory {
        ...
        public static var outputHelp: String {
            isChinese
                ? "輸出檔案路徑（預設：ORRERY_MEMORY.md）"
                : "Output file path (default: ORRERY_MEMORY.md)"
        }
        ...
        public static var storageAbstract: String {
            isChinese
                ? "設定 memory 的儲存路徑（預設為 ~/.orrery）"
                : "Set the memory storage path (default: ~/.orrery)"
        }
        ...
        public static var storageResetHelp: String {
            isChinese ? "還原為預設儲存路徑（~/.orrery）" : "Reset to default storage path (~/.orrery)"
        }
        ...
        public static func storageStatus(_ path: String?) -> String {
            ...
            return isChinese ? "儲存路徑：預設（~/.orrery）" : "Storage: default (~/.orrery)"
        }
        ...
        public static var storageReset: String {
            isChinese ? "已還原為預設儲存路徑（~/.orrery）。" : "Memory storage reset to default (~/.orrery)."
        }
    ...
    public enum MCPSetup {
        ...
        public static var setupAbstract: String {
            isChinese
                ? "在當前專案設定 Orrery MCP server"
                : "Set up Orrery MCP server for the current project"
        }
        public static var success: String {
            isChinese
                ? "Orrery MCP server 與 slash commands 已設定完成。重新啟動 AI tool 即可使用 /orrery:delegate、/orrery:sessions 和 /orrery:resume。"
                : "Orrery MCP server and slash commands installed. Restart your AI tool to use /orrery:delegate, /orrery:sessions, and /orrery:resume."
        }
